...
function setupEventListeners() {
  document.getElementById("search")?.addEventListener("input", debounce(renderWashTable, 300));
  document.getElementById("filterStatus")?.addEventListener("change", renderWashTable);
  document.getElementById("btnSaveWash")?.addEventListener("click", saveWashJob);
  ...
  document.getElementById("color")?.addEventListener("change", autofillEmployeeInfo);
  document.addEventListener("keydown", (e) => {
    if (e.key === "Escape") toggleModal(false);
  });
  document.getElementById("btnAddWash")?.addEventListener("click", openAddWashModal);
...
async function saveWashJob() {
  ...
  try {
    ...
    if (rewashCount > 3) {
      const scrapData = {
        washId, empId, empName, uniformCode, color, size,
        createdAt: new Date().toISOString(),
        status: "Scrap", rewashCount, testResult: "FAIL", testDate: new Date().toISOString(),
      };
    ...
    const newWash = {
      washId, empId, empName, uniformCode, color, size,
      createdAt: new Date().toISOString(), status, rewashCount,
    };
      ...
      if (wash?.status?.includes("Waiting")) {
        ...
        if (snap.length > 0) {
          await updateDoc(doc(db, "InventoryDB", snap[0].id), {
            status: "in-use", rewashCount: wash.rewashCount || 0,
          });
        }
      }
...
export async function markAsESDPass(washData) {
  try {
    ...
    await addToWashHistory({ ...washData, testResult: "PASS", testDate: new Date().toISOString(), status: "ESD Passed" });
    await returnToStockAfterESD({ ...washData, rewashCount: 0, status: "ESD Passed" });
...
export async function markAsESDFail(washData) {
  try {
    ...
    await addToWashHistory({ ...washData, testResult: "FAIL", testDate: new Date().toISOString(), status: "ESD Failed" });
...
// ============================ 📤 EXPORT CSV ============================
document.getElementById("btnExportCSV")?.addEventListener("click", exportWashToCSV);
...
document.getElementById("btnExportWashHistoryCSV")?.addEventListener("click", exportWashHistoryToCSV);
